...
public class PromisesTest {
    ...
    public void testPromises() throws Exception {
		...
		promise.select(promise2).foreach(v -> result3.set("Selected: " + v));
		final Promise<String> result4 = new Promise<>();
	    promise2.select(promise).foreach(v -> result4.set("Selected: " + v));
        ...
        promise.select(promise4).foreach(s -> result1.set("Selected: " + s));
	    ...
	    Promise<Pair<String, String>> join = promise3.join(onraise.onRaise(e -> {
			synchronized (monitor) {
	      		monitor.countDown();
	    	}
		}));
	    onraise.onRaise(e -> { synchronized (monitor) {
	      monitor.countDown();
	    }});
	    ...
	    promise.foreach(v -> result9.set("Foreach: " + v));
